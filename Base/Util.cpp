...
{
    constexpr auto BUILD_DAY = (\
        (__DATE__[4] == ' ' ? 0 : __DATE__[4] - '0') * 10 + \
        (__DATE__[5] - '0') \
        );

    constexpr auto BUILD_MONTH = (\
    ...
    return fmt::format("SimCoupe v{}.{}.{}{}  {}-{:02}-{:02}",
        SIMCOUPE_MAJOR_VERSION, SIMCOUPE_MINOR_VERSION, SIMCOUPE_PATCH_VERSION,
#ifdef _WIN64
        " x64",
#else
        "",
